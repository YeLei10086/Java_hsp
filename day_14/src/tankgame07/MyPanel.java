...
public class MyPanel extends JPanel implements KeyListener, Runnable {
    ...
    public MyPanel(String key) {
        ...
        if (file.exists()) {
            nodes = Recorder.tanksNodesAndShotEnemyTankNum();
        }else {
        ...
        switch (key){
            case "1":   //开启新游戏
                Recorder.setShotEnemyTankNum(0);
                for (int i = 0; i < enemyTankSize; i++) {
                    //添加敌方坦克,x轴变化,y轴不变
                    EnemyTank enemyTank = new EnemyTank(100 * (i + 1), 0);
                    //将 enemyTanks 设置给 enemyTank,否则集合中没有比较的坦克
        ...
        //加入坦克游戏bgm
        new AePlayWave("day_14\\src\\111.wav").start();
    }
    ...
    public void showInfo(Graphics g){
        ...
        g.setFont(font);
        g.drawString("击毁坦克数据记录",1040,30);

        drawTank(1020,60,g,0,1);
        g.setColor(Color.BLACK);
        g.drawString(Recorder.getAllEnemyTank() + "",1070,100);
    }
